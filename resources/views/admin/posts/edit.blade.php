        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-white rounded-xl p-5 shadow-sm">
                <div class="flex items-center justify-between mb-2">
                    <label class="font-medium text-gray-700">المقتطف</label>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <select name="platform" class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                            @foreach(['pc' => 'كمبيوتر', 'android' => 'أندرويد', 'ios' => 'iOS', 'mac' => 'ماك', 'all' => 'جميع المنصات'] as $val => $label)
                    ...
                    <div>
                        <label class="text-sm font-medium text-gray-600 mb-1 block">آخر تحديث</label>
            ...
            <div class="bg-white rounded-xl p-5 shadow-sm space-y-4">
                ...
                @foreach([
                    ['features', 'المميزات'],
                    ['system_requirements', 'متطلبات التشغيل'],
                    ['whats_new', 'ما الجديد'],
                ] as [$field, $label])
                <div>
                    <label class="text-sm font-medium text-gray-600 mb-1 block">{{ $label }}</label>
            ...
            <div class="bg-white rounded-xl p-5 shadow-sm space-y-4">
                ...
                @foreach([
                    ['meta_title', 'عنوان Meta (max 60)', 'text', 'meta_title'],
                    ['meta_description', 'وصف Meta (max 155)', 'textarea', 'meta_description'],
                    ['meta_keywords', 'الكلمات المفتاحية', 'text', 'meta_keywords'],
                ] as [$field, $label, $inputType, $aiAction])
                <div>
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <select name="status" class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                            @foreach(['draft' => 'مسودة', 'pending' => 'قيد المراجعة', 'published' => 'منشور', 'scheduled' => 'مجدول'] as $val => $label)
                    ...
                    <div>
                        ...
                        <select name="type" class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                            @foreach(['game' => 'لعبة', 'software' => 'برنامج', 'apk' => 'تطبيق أندرويد', 'blog' => 'مقال', 'tutorial' => 'شرح', 'listicle' => 'قائمة', 'review' => 'مراجعة'] as $val => $label)
                            <option value="{{ $val }}" {{ old('type', $post->type) === $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="text-sm font-medium text-gray-600 mb-1 block">Slug</label>
