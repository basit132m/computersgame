...
@section('title', 'تعديل تحويل')

@section('content')
<div class="max-w-2xl">
    <form action="{{ route('admin.redirects.update', $redirect) }}" method="POST" class="space-y-4">
        @csrf @method('PUT')
        @if($errors->any())
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="type" class="w-full px-4 py-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="301" {{ old('type', $redirect->type) == 301 ? 'selected' : '' }}>301 — دائم</option>
                        <option value="302" {{ old('type', $redirect->type) == 302 ? 'selected' : '' }}>302 — مؤقت</option>
                    </select>
                </div>
                <div class="flex items-end pb-1">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="active" value="1" {{ old('active', $redirect->active) ? 'checked' : '' }}
